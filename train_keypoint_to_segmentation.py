...
def train_model(train_source_iter, device, student, seg_model, model, optimizer, criterion, epoch, visualize, args):
    ...
    for i in range(iters_per_epoch):
        ...
        with torch.cuda.amp.autocast():
            y_s = student(x_s)  # B, 16, 64, 64, input: source image, output:  source heatmap
            
            seg_x_s = get_segmentation_masks(seg_model, x_s)  # B, num_classes, 256, 256, deeplab segmentation mask
            seg_x_s = torch.argmax(seg_x_s, dim=1).float()  # B, 256, 256,
            #seg_x_s = (seg_x_s > args.seg_threshold).float().unsqueeze(1)
